            ...
            <div className="space-y-3 w-full">
              ...
              <div className="flex gap-2 flex-wrap">
                {['ALL', ...leaseCompanies].map(l => (
                  <button key={l} onClick={() => { setSelectedLeaseCo(l); setActiveView({ branchName: null, secondName: null, thirdName: null }); }}
  ...
  const totalRow = useMemo(() => {
    ...
    filtered.forEach(item => {
      monthCols.forEach(m => {
        ...
        quantity[m] += item.quantity?.[m] || 0;
      });
    });
    let sumS = 0, sumP = 0, sumC = 0, sumQ = 0;
    monthCols.forEach(m => { sumS += sales[m]; sumP += profit[m]; sumC += count[m]; sumQ += quantity[m]; });
    sales['計'] = sumS;
    profit['計'] = sumP;
    count['計'] = sumC;
    quantity['計'] = sumQ;
            ...
            {totalRow && (
              <tr className={`bg-red-50/50 border-b-2 border-red-200 ${!isLeafLevel && !isProductMode && !activeView.branchName ? 'cursor-pointer hover:bg-red-100/50 transition-colors' : ''}`} onClick={() => { if (!isLeafLevel && !isProductMode && !activeView.branchName) onDrillDown({ name: '__ALL__' }); }}>
                ...
                {months.map((month, mIdx) => {
                  ...
                  if (month === '計') {
                    s = 0; p = 0; c = 0; q = 0;
                    monthCols.forEach(m => { s += totalRow.sales[m] || 0; p += totalRow.profit[m] || 0; c += totalRow.count?.[m] || 0; q += totalRow.quantity?.[m] || 0; });
                  } else {
                    s = totalRow.sales[month] || 0;
                    p = totalRow.profit[month] || 0;
                    c = totalRow.count?.[month] || 0;
                    q = totalRow.quantity?.[month] || 0;
                  }
                  const prevMonth = months[mIdx - 1];
                  const yoy = prevMonth && month !== '計' ? calcYoY(s, totalRow.sales[prevMonth]) : null;
                  const qoy = prevMonth && month !== '計' ? calcYoY(q, totalRow.quantity?.[prevMonth]) : null;
                  return (
                    <td key={month} className="px-2 md:px-6 py-4 border-l border-red-200">
                      <div className="space-y-2">
                        {isProductLevel ? (
                          <div className="flex justify-between items-baseline">
                            <span className="text-[10px] font-black text-red-500">数量</span>
                            <div className="text-right">
                              <div className="font-mono font-black text-red-800 text-xs md:text-base">{q.toLocaleString()}</div>
                              {qoy !== null && <div className={`text-[10px] font-black flex items-center justify-end gap-0.5 ${parseFloat(qoy)>=0?'text-emerald-500':'text-rose-500'}`}>{parseFloat(qoy)>=0?<ArrowUpRight size={10}/>:<ArrowDownRight size={10}/>}{qoy}%</div>}
                            </div>
                          </div>
                        ) : (
                          <>
              ...
              data.map((item, idx) => (
                <tr key={idx} className={`group hover:bg-red-50/30 transition-all ${!isLeafLevel && !isCustomerSearchMode?'cursor-pointer':''}`} onClick={() => !isLeafLevel && !isCustomerSearchMode && onDrillDown(item)}>
                  ...
                  {months.map((month, mIdx) => {
                    ...
                    if (month === '計') {
                      s = 0; p = 0; c = 0; q = 0;
                      monthCols.forEach(m => { s += item.sales[m] || 0; p += item.profit[m] || 0; c += item.count?.[m] || 0; q += item.quantity?.[m] || 0; });
                    } else {
                      s = item.sales[month] || 0;
                      p = item.profit[month] || 0;
                      c = item.count?.[month] || 0;
                      q = item.quantity?.[month] || 0;
                    }
                    const prevMonth = months[mIdx - 1];
                    const yoy = prevMonth && month !== '計' ? calcYoY(s, item.sales[prevMonth]) : null;
                    const qoy = prevMonth && month !== '計' ? calcYoY(q, item.quantity?.[prevMonth]) : null;
                    return (
                      <td key={month} className="px-2 md:px-6 py-4 border-l border-slate-300 group-hover:bg-white/50">
                        <div className="space-y-2">
                          {isProductLevel ? (
                            <div className="flex justify-between items-baseline">
                              <span className="text-[10px] font-black text-slate-600">数量</span>
                              <div className="text-right">
